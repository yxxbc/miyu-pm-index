...
def write_index(out: Path, entries: Dict[str, Dict[str, Any]]) -> None:
    ...
    write_packages_md(out, packages)


def write_packages_md(out: Path, packages: List[Dict[str, Any]]) -> None:
    lines = [
        "# 📦 Available Packages",
        "",
    ...
    for p in packages:
        ...
        repo = (p.get("repo") or "-").replace("|", "\\|")
        desc = (p.get("description") or "").replace("\n", " ").replace("|", "\\|")
        lines.append(f"| `{name}` | {kind} | {version} | {repo} | {desc} |")
    lines.extend(["", "_Auto-generated by collect-registry_", ""])
    (out / "PACKAGES.md").write_text("\n".join(lines), encoding="utf-8")
    print(f"wrote PACKAGES.md with {len(packages)} package(s)")
